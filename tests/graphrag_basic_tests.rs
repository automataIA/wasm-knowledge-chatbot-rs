...
async fn test_retrieval_rerank_flag_sets() {
    ...
    assert!(r1.metadata.reranked, "expected reranked=true when use_reranking is set");
    ...
    assert!(!r2.metadata.reranked, "expected reranked=false when use_reranking is not set");
}
...
async fn test_state_searching_and_result_flow() {
    ...
    assert!(!ctx.searching_now(), "searching should be false after completion");
    assert!(ctx.last_result_now().is_some(), "last_result should be set after search");
}
