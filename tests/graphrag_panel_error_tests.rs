...
use wasm_knowledge_chatbot_rs::state::{GraphRAGStateProvider, GraphRAGStateContext};
...
fn ErrorSetter() -> impl IntoView {
    ...
    Effect::new(move |_| {
        ctx.set_error(Some(AppError::GraphRAGError("Forced error for test".into())));
    });
    view! { <div></div> }
}
...
fn graph_rag_panel_renders_error_message() {
    mount_to_body(|| view! { <GraphRAGStateProvider><ErrorSetter/><GraphRAGPanel/></GraphRAGStateProvider> });

    let d = doc();
    let body_text = d.body().unwrap().text_content().unwrap_or_default();

    // Should render user-friendly error section (warning alert)
    assert!(body_text.contains("GraphRAG"), "should include GraphRAG text from user-friendly error message");
}
