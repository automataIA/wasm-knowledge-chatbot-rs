...
fn mount_panel_and_check_core_elements() {
    ...
    // Input exists
    assert!(d.query_selector("input.input").unwrap().is_some(), "query input should exist");

    // Search button exists and is a button
    assert!(d.query_selector("button.btn.btn-primary").unwrap().is_some(), "search button present");

    // Reindex button exists
    assert!(d.query_selector("button.btn.btn-ghost").unwrap().is_some(), "reindex button present");

    // Reranking checkbox exists
    assert!(d.query_selector("input.checkbox").unwrap().is_some(), "reranking checkbox present");

    // Strategy select exists
    assert!(d.query_selector("select.select").unwrap().is_some(), "strategy select present");
    ...
    assert!(body_text.contains("Enter a query and press Search"), "empty state hint should be visible");
}
