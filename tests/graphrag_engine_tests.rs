...
use wasm_knowledge_chatbot_rs::advanced_graphrag::{HyDEConfig, HyDEEngine, CommunityDetectionConfig, CommunityDetectionEngine};
use wasm_knowledge_chatbot_rs::pagerank_reranking::{PageRankConfig, PageRankEngine, RerankingConfig, AdvancedReranker, SynthesisConfig, ResultSynthesizer, GraphAccess};
...
fn community_two_clusters_with_sparse_cross_edges() {
    ...
    for c in &mut comms { c.sort_unstable(); }
    comms.sort();
...
fn community_detects_two_clusters() {
    ...
    let mut sets: Vec<Vec<usize>> = comms.into_iter().map(|mut c| { c.sort_unstable(); c }).collect();
    sets.sort();
...
fn pagerank_converges_and_normalizes() {
    ...
    struct SimpleGraph { adj: Vec<Vec<usize>> }
    ...
    for u in 0..n { adj[u].push((u+1)%n); }
    let g = SimpleGraph { adj };

    let cfg = PageRankConfig { damping: 0.85, iterations: 200, convergence: 1e-8, ..Default::default() };
    ...
    for s in scores { assert!((s - 1.0/n as f32).abs() < 1e-3); }
}
...
fn community_partition_invariants() {
    ...
    let n = 6usize;
    let g = G { adj: vec![vec![1], vec![2], vec![3], vec![4], vec![5], vec![0]] };
    let engine = CommunityDetectionEngine::new(CommunityDetectionConfig::default());
    let comms = engine.detect_communities(&g);
...
fn reranker_weight_effects() {
    ...
    let cfg = RerankingConfig { weight_mono_t5: 1.0, weight_tildev2: 0.0, weight_original: 0.0 };
...
fn pagerank_star_graph_distribution() {
    ...
    for i in 2..n { assert!((scores[i] - leaf_score).abs() < 1e-4); }
}
...
fn pagerank_with_dangling_nodes_normalizes() {
    ...
    let sum: f32 = scores.iter().sum();
    assert!((sum - 1.0).abs() < 1e-4);
    for s in scores { assert!(s.is_finite() && s >= 0.0); }
}
